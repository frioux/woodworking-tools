...
function renderStickFigure(doc, model, theta, geom) {
  ...
  // Backrest X-position at a given local Y.  Guard against near-horizontal
  // backrest values to keep posture solving numerically stable.
  const backrestXAtY = (y) => {
    const yRel = y - backBaseY;
    const sinBack = Math.sin(backAngleRad);
    if (Math.abs(sinBack) < 1e-6) {
      return backBaseX;
    }
    return backBaseX + yRel * (Math.cos(backAngleRad) / sinBack);
  };
  ...
  const signedBackrestDistance = (x, y) => x - backrestXAtY(y);
